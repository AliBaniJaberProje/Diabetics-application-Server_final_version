    ...
    try{
        ...
        const events=await event.find({$and:[
            {idDoctor:resultDecodeJWT.id}, {startEventTime:{ $gte: startDate, $lte: endDate }}
                ...
                ]}).populate("taken.patientRef").select({_id:true,startEventTime:true,endEventTime:true,title:true,typeEvent:true,taken:true,"taken.patientRef.imgURL":true,"taken.patientRef.id":true})
