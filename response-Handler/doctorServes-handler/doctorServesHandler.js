...
const getAllPatientForDoctor=async (req,res,_)=>{
    ...
    try {
       ...
       // const resultDecodeJWT= await jwt.decode(req.headers['token']);
        ...
        const patuiontResult=await Patient.find({"currentDoctor":resultDecodeJWT.id}," id ").populate(" patient ","active").select({imgURL:true,_id :false,username:true,phoneToken:true})
