...
const addNewPatient=async (req,res,_)=>{
    ...
    try{
        ...
        client1.messages
            .create({
                body: "رابط تنزيل التطبيق" +"____________________"+
...
const getAllPatientForDoctor=async (req,res,_)=>{
    ...
    try {
       ...
       // const resultDecodeJWT= await jwt.decode(req.headers['token']);
        ...
        const patuiontResult=await Patient.find({"currentDoctor":resultDecodeJWT.id},).select({imgURL:true,_id :false,username:true,phoneToken:true,isOnline:true,id:true})
